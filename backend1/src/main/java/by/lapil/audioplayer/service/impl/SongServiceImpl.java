...
public class SongServiceImpl implements SongService {
    ...
    public void parseAll() {
        // Путь к папке ресурсов (предполагаем, что ресурсы находятся в src/main/resources)
        File resourceDir = new File("backend/src/main/resources/songs");

        if (!resourceDir.exists() || !resourceDir.isDirectory()) {
            logger.error("Папка resources не найдена.");
            return;
        }
        ...
        if (musicFiles == null || musicFiles.length == 0) {
            logger.info("Нет музыкальных файлов в папке.");
            return;
        }
        ...
        for (File file : musicFiles) {
            ...
            if (parts.length < 2) {
                logger.info("Неверный формат имени файла");
                continue;
            }
            ...
            List<ArtistDto> artistDtos = artistService.createBulk(createArtistDtoList);
            ...
            artists.forEach(artist -> artist.getSongs().add(song));
